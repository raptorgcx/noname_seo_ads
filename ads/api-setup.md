...
> **СТАТУС 2026-07-14 (вечер): все 4 компонента СОБРАНЫ за один вечер.**
> Кабинет 799-748-9829 (RON, карта, промо +1500 RON) · MCC 489-661-0695, привязан ·
> developer token получен, заявка на Basic отправлена · OAuth client (Desktop) создан,
> refresh token получен loopback-флоу, привязочный вызов listAccessibleCustomers
> УСПЕШЕН (вернул оба аккаунта) · **Brand verification ПРОЙДЕНА** (страница
> nonamelounge.ro/ads-automation/ создана под требования: имя, назначение, privacy/terms;
> робот добавлен владельцем домена в GSC), branding опубликован → заявка в ускоренной
> очереди (часы вместо 5 дней). Все креды: .secrets/google-ads.env.
> Осталось: дождаться письма об одобрении Basic → перейти на google-ads-python.
